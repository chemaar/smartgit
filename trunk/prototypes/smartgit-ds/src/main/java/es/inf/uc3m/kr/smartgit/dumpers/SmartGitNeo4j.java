...
//	}
//	
	public static List<String> getInternalUserIds(String userLogin){
		String query = "match (n:USER_NODE) where n.Login={login} return id(n);";
		Map<String, Object> params = new HashMap<String, Object>();
		params.put( "login", userLogin );
		return runQuery(Neo4jDatabaseConnector.getGraphDatabaseService(),query, params, "id(n)");
	}
	
	public static List<String> getInternalRepoIds(String repoId){
	...
	private static List<String> runQuery(GraphDatabaseService graphDb,String query, Map<String, Object> params,String var) {
		List<String> internalIds = new LinkedList<String>();
		ExecutionEngine engine = new ExecutionEngine(graphDb);
		try (Transaction tx = graphDb.beginTx()) {
			ExecutionResult result = engine.execute(query, params);
			// extract the data out of the result, you cannot iterate over it outside of a tx
            ...
            for ( Long id : IteratorUtil.asIterable(n_column ) ) {
                ...
                internalIds.add(String.valueOf(id));
            }
			tx.success();
		}
		return internalIds;
	...
	public static void main(String []args){
		String userLogin = "chemaar";
		List<String> ids = getInternalUserIds(userLogin);
		for(String id:ids){
